...
origination_blurring_plot = function(t = seq(0, 3.5, by = 0.1),
                                     lambdas = 2^seq(-2, 3)){
  # convolution
  ...
  for (lambda in lambdas){
    df = rbind(df, data.frame(orig = orig_blur_vector(f_orig, lambda, t), t = t, lambda = rep(lambda, length(t))))
  }
  # add "true" rate for plotting
  df = rbind(df, data.frame(orig = df_true_orig_rate$y,
                            t = df_true_orig_rate$x,
                            lambda = rep(Inf, length(df_true_orig_rate$x))))
  df$lambda = factor(df$lambda, levels = c(lambdas, Inf))
  
  # black for true rate
  cols = setNames(c(scales::hue_pal()(length(lambdas)), "black"), levels(df$lambda))
...
extinction_blurring_plot = function(t = seq(0, 3.5, by = 0.1),
                                    lambdas = 2^seq(-2, 3)){
  ...
  df = rbind(df, data.frame(ext = df_true_ext_rate$y,
...
line_plot_full = function(lambda,
                          f_ext,
                          f_orig,
                          n_taxa,
                          t_range,
  ...
  while (length(li) < n_taxa){
    ...
    e = StratPal::p3_var_rate(f_ext,
                              from = min(t),
                              to = max(t),
                              n = 1,
                              f_max = 50)
    origin = StratPal::p3_var_rate(f_orig,
                                   from = min(t),
                                   to = max(t),
                                   n = 1,
                                   f_max = 50)
    if (e <= origin){next} # skip if contradcition
    # simulate fossil samples
    x = StratPal::p3(rate = lambda, # sampling frequency
                     from = origin,
                     to = e)
    if (length(x) > 1){ # enforce at least 2 fossils, excludes singletons
  ...
  df_ext_true = data.frame(taxon = factor(names_tax),
                           t_ext = true_ext)
  df_ext_obs = data.frame(taxon = factor(names_tax),
                          last_occ = last_occ)
  df_orig_true = data.frame(taxon = factor(names_tax),
                            t_orig = true_origin)
  df_orig_obs = data.frame(taxon = factor(names_tax),
                           first_occ = first_occ)
  df_unobs_range_top = data.frame(taxon = factor(rep(names_tax, 2)),
                                  lim = c(true_ext, last_occ))
  df_unobs_range_bottom  = data.frame(taxon = factor(rep(names_tax, 2)),
  ...
  p =   ggplot(df_ext_true, aes(x = taxon, y = t_ext)) +
    geom_rect(data = bg_fill_ext, inherit.aes = FALSE, # extinction rate background fill
    ...
    new_scale_fill() +
    geom_rect(data = bg_fill_orig, inherit.aes = FALSE, # origination rate background fill
              aes(ymin = ymin, ymax = ymax, xmin = -Inf, xmax = Inf, fill = orig)) + scale_fill_gradient(
                low = "white",
                high = "darkorange",                                                           name = "True\norig. rate\n[taxa/Myr]",
                guide = guide_colorbar(order = 2)) +
    geom_line(data = df_obs_range, aes(x = taxon, y = lim)) + # observed range
    geom_line(data = df_unobs_range_top, aes(x = taxon, y = lim), linetype = "dashed") + # unobserved ranges
    geom_line(data = df_unobs_range_bottom, aes(x = taxon, y = lim), linetype = "dashed") +
    ...
    geom_point(data = df_orig_true, aes(x = taxon, y = true_origin), shape = 4) + # true orig
    geom_point(data = df_orig_obs, aes(x = taxon, y = first_occ), color = "red") + # first occ
...
p = (p1 + small_legend | p2 + small_legend) / p3 +
